...
- 只想统计部门管理的 `list` 方法执行耗时，就只有这该方法是原始业务方法。
- 想统计所有部门管理的业务方法执行耗时，就所有的部门管理的业务方法都是原始业务方法。
...
- 比如：入门程序当中，所有的业务方法都可以被 AOP 控制，它们都是连接点。
...
- @Around 环绕通知，环绕后的代码逻辑也不会再执行了 ，因为原始方法调用已经出异常了。
