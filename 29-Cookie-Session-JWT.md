...
1. 使用浏览器，访问 `http://localhost:8080/c1`，打开浏览器开发者工具：
   - 观察服务器端返回的**响应标头**中 `Set-Cookie` 属性（服务器设置的 Cookie 返回给浏览器）。
2. 在浏览器访问 `http://localhost:8080/c2`，打开开发者工具；
   - 观察浏览器发送的**请求标头**中的 `Cookie` 属性（浏览器携带的 Cookie）。
...
在浏览器中，访问 `http://localhost:8080/s1`，打开开发者工具，观察服务器端返回的响应标头：
...
在浏览器中，访问 `http://localhost:8080/s2`，打开开发者工具，观察浏览器发送的请求标头：
...
> Base64 是一种基于 64 个可打印的字符，来表示二进制数据的编码方式。
>
> - 使用的 64 个字符分别是 A-Z、a-z、0-9，一个加号（+），一个斜杠（/）；
> - 还有一个等号。它是一个补位的符号；
...
- 会对第一、第二部分的 JSON 格式数据，进行 Base64 编码。反之它们可以通过 Base64 解码得到。
- 第三部分，会**指定密钥**，使用签名算法计算得到。无法被 Base64 解码。

### 3.4.JWT 的使用场景

JWT 令牌，最典型的应用场景，就是登录认证：

1. 客户端发起请求执行登录操作，访问服务端的登录接口，如果登录成功，服务端需要生成一个 jwt 令牌，返回给客户端。
2. 客户端拿到 jwt 令牌之后，将它存储在本地。在后续的每一次请求中，都将 jwt 令牌携带到服务端。
3. 服务端统一拦截请求，先来判断该请求是否携带令牌；否，直接拒绝访问；是，还要校验一下令牌是否是有效；否，则拒绝访问；是，就放行请求；
