...
前面介绍了基于令牌技术来实现会话追踪。这里的令牌就是用户身份的标识，其本质是一个字符串。
...
生成 JWT 时，会对前两部分的 JSON 格式的数据，进行 Base64 编码。所以它们可以通过 Base64 解码得到。
...
1. 客户端发起请求来执行登录操作，访问服务端的登录接口，如果登录成功，服务端需要生成一个jwt令牌，返回给客户端端。
2. 客户端拿到 jwt 令牌之后，将它存储起来。在后续的每一次请求中，都将 jwt 令牌携带到服务端。
