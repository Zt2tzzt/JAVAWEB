...
接下来介绍 Maven 提供的一些高级功能，这些功能在构建和管理 Java 项目时，用的也是非常多。
...
分模块设计，指的就是在设计一个 Java 项目的时候，将它拆分成多个模块进行开发。

![Maven分模块设计](NoteAssets/Maven分模块设计.png)

如果项目不分模块，就意味着所有的业务代码，都写在这一个项目当中。

- 随着这个项目的业务扩张，项目中的业务功能，会越来越多。

如果开发的是一个大型的电商项目，里面可能包含了商品模块、搜索模块、购物车模块、订单模块、用户中心模块等等。

这些所有的业务代码都在一个 Java 项目当中编写。加上大量开发人员同时操作一个项目，有两大问题：

- 项目管理和维护起来将会非常的困难。
- 项目中，定义的一些通用工具类、组件，复用非常不方便。

分模块设计，就是在项目设计阶段，将一个大的项目，拆分成若干个模块，每一个模块都是独立的。

![Maven分模块设计 (2)](NoteAssets/Maven分模块设计 (2).png)

- 将商品相关业务功能，放在商品模块；搜索相关业务功能，放在搜索模块；购物车模块、订单模块…类似。
- 为了组件的复用，可以将项目当中的实体类、工具类，通用组件，单独的抽取到一个模块当中。

这样就将一个项目，拆分成了若干个模块儿，这就是分模块儿设计。

- 进行项目管理的时候，就可以项目组为单位，负责不同模块的开发，这样更加便于项目的管理、维护。
- 当前模块（比如订单模块）需要用到这些实体类、工具类、或通用组件，直接引入通用模块坐标即可。

分模块设计，就是将项目，按照功能、结构，拆分成若干个子模块；
...
- 方案一：直接依赖当前项目 javaweb-practise，存在两大缺点：
...
- 方案二：分模块设计：
  ...
  - 将 pojo 包下的实体类，抽取到一个 maven 模块中 javaweb-pojo
  - 将utils包下的工具类，抽取到一个maven模块中 javaweb-utils
  - 其他的业务代码，仍放在 javaweb-practise 这个模块中，在该模块中引入  javaweb-pojo、 javaweb-utils 依赖。

![javaweb分模块案例理解](NoteAssets/javaweb分模块案例理解.png)

注意：分模块开发需要先针对模块功能进行设计，再进行编码。而不是先将工程开发完毕，然后进行拆分。
...
删除原项目的 utils 包，然后在 pom.xml中引入 javaweb-utils 的依赖
